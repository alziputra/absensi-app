...
export default function Dashboard() {
  ...
  const checkAbsenMasukHariIni = async (uid) => {
    const q = query(
      ...
      where("userId", "==", uid),
      where("tipe_absen", "==", "Absen Masuk")
    );
    const snapshot = await getDocs(q);
    const todayStr = new Date().toDateString();
    
    // Filter data Firestore menggunakan Javascript untuk mencari tanggal hari ini
    const sudahAbsen = snapshot.docs.some(doc => {
  ...
  const handlePhotoCapture = async (e) => {
    ...
    try {
      ...
      if (absenType === 'Absen Masuk') {
        ...
        batasJam.setHours(8, 0, 0, 0); // Batas masuk Jam 08:00 Pagi

        if (now > batasJam) {
          statusTelat = "Terlambat";
          const diffMs = now - batasJam;
          const diffHrs = Math.floor(diffMs / 3600000);
          const diffMins = Math.floor((diffMs % 3600000) / 60000);
          detailTelat = `${diffHrs} Jam ${diffMins} Menit`;
        }
      }
