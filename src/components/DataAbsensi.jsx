...
export default function DataAbsensi() {
  ...
  // --- FUNGSI EXPORT EXCEL/CSV GABUNGAN ---
  const handleExportCSV = () => {
    ...
    logs.forEach((log) => {
      ...
      if (log.tipe_absen === "Absen Masuk") {
        groupedData[tgl].masuk = log.waktuObj.toLocaleTimeString("id-ID");
        ...
        groupedData[tgl].pulang = log.waktuObj.toLocaleTimeString("id-ID");
        groupedData[tgl].lokasiPulang = log.lokasi || "Tanpa GPS";
      }
    });

    // 2. Susun Struktur Header CSV
    const headers = [
      "Tanggal",
      "Jam Masuk",
      "Status",
    ...
    // 3. Masukkan data yang sudah dikelompokkan ke baris CSV
    Object.values(groupedData).forEach((row) => {
      const csvLine = [
        ...
        row.status,
        row.telat,
        `"${row.lokasiMasuk}"`, // Gunakan tanda kutip agar koma di koordinat tidak merusak kolom
        row.pulang,
        `"${row.lokasiPulang}"`,
      ];
    ...
    link.href = csvUrl;
    link.download = `Laporan_Absensi_Lengkap_${new Date().toLocaleDateString("id-ID")}.csv`;
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans relative overflow-x-hidden pb-20">
      ...
      <div
        className={`fixed top-0 right-0 h-full w-64 bg-[#1a1c23] text-white z-50 flex flex-col transform transition-transform duration-300 ease-in-out ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}
      >
      ...
      <main className="max-w-6xl mx-auto p-6 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center my-8 gap-4">
          ...
          <button
            ...
            className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-semibold flex items-center gap-2 text-sm transition-colors shadow-sm"
          >
            <svg
              className="w-5 h-5"
              fill="none"
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-4 text-sm text-gray-800">
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-semibold ${log.status_kehadiran === "Terlambat" ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"}`}
                            >
